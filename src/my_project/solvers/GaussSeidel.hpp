...
namespace Slae::Solvers {
    template<typename T>
    std::vector<T> GaussSeidelIter(const Slae::Matrix::CSR<T> &A, const std::vector<T> &b, std::vector<T> x) {
    ...
    template<typename T>
    std::vector<T> GaussSeidel(const Slae::Matrix::CSR<T> &A, const std::vector<T> &b, const std::vector<T> &initialState,
                const T &tolerance) {
        if (A.rows_number() != b.size()) {
            ...
            auto norma = norm(r, NormType::ThirdNorm);
    ...
    template<typename T>
    std::vector<T> FastGaussSeidel(const Slae::Matrix::CSR<T> &A, const std::vector<T> &b, const std::vector<T> &initialState, T rho,
                    const T &tolerance) {
        if (A.rows_number() != b.size()) {
            std::stringstream buf;
            buf << "Matrix has unequal sizes! Matrix size: " << A.rows_number() << ". Col size: " << A.cols_number()
